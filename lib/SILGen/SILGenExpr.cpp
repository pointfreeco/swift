...
static SILFunction *getOrCreateKeyPathEnumCaseExtract(
    SILGenModule &SGM, EnumElementDecl *element, SubstitutionMap subs,
    GenericEnvironment *genericEnv, ResilienceExpansion expansion,
    CanType baseType, CanType componentType) {
  auto genericSig =
      genericEnv ? genericEnv->getGenericSignature().getCanonicalSignature()
                 : nullptr;
  if (genericSig && genericSig->areAllParamsConcrete()) {
  ...
  auto signature = createKeyPathSignature(
      SGM, genericSig, baseType, componentType, {},
      SILFunctionType::Representation::KeyPathAccessorGetter,
      ParameterConvention::Indirect_In_Guaranteed);

  auto name = Mangle::ASTMangler(SGM.getASTContext(), element)
                  .mangleKeyPathCaseExtractThunkHelper(element, genericSig,
                                                       baseType, subs,
                                                       expansion);
  ...
  if (genericEnv) {
    baseType = genericEnv->mapTypeIntoEnvironment(baseType)->getCanonicalType();
    componentType =
        genericEnv->mapTypeIntoEnvironment(componentType)->getCanonicalType();
    thunk->setGenericEnvironment(genericEnv);
  }
  ...
  SILFunctionArgument *baseArg = nullptr;
  SILValue indexPtrArg;
  emitKeyPathThunk(SGM, subSGF, genericEnv, signature, thunk, {}, resultArg,
                   resultArgTy, baseArg, baseArgTy, indexPtrArg,
                   SILParameterInfo());
  ...
  CanType payloadType = componentType.getOptionalObjectType();
  assert(payloadType && "enum case component must project an Optional");

  ArgumentScope scope(subSGF, loc);

  auto &B = subSGF.B;
  auto enumTy = subSGF.getLoweredType(baseType);
  auto optTy = subSGF.getLoweredType(componentType);
  auto payloadTy = subSGF.getLoweredType(payloadType);
  bool useLoweredAddresses = SGM.M.useLoweredAddresses();
  bool resultIsAddressOnly =
      optTy.isAddressOnly(subSGF.F) && useLoweredAddresses;
  bool isIndirect =
      element->isIndirect() || element->getParentEnum()->isIndirect();
  // Boxed (indirect) payloads are extracted through the address-based
  // path as well, which projects the box from the taken enum data.
  bool subjectIsAddressOnly =
      (enumTy.isAddressOnly(subSGF.F) || isIndirect) && useLoweredAddresses;
  bool hasPayload = element->hasAssociatedValues();
  bool payloadIsAddressOnly =
      hasPayload && payloadTy.isAddressOnly(subSGF.F) && useLoweredAddresses;
  SILType eltArgTy;
  if (hasPayload)
    eltArgTy = enumTy.getEnumElementType(element, SGM.M,
                                         subSGF.getTypeExpansionContext());

  // Allocate the buffers needed below up front, so that stack discipline
  // is maintained across the control-flow merge.
  SILValue resultTmp;
  if (resultIsAddressOnly)
    resultTmp = subSGF.emitTemporaryAllocation(loc, optTy);
  SILValue subjectTmp;
  if (subjectIsAddressOnly)
    subjectTmp = subSGF.emitTemporaryAllocation(loc, enumTy);
  SILValue payloadTmp;
  if (payloadIsAddressOnly)
    payloadTmp = subSGF.emitTemporaryAllocation(loc, payloadTy);

  auto *matchBB = subSGF.createBasicBlock();
  auto *defaultBB = subSGF.createBasicBlock();
  auto *contBB = subSGF.createBasicBlock();
  if (!resultIsAddressOnly) {
    contBB->createPhiArgument(optTy.getObjectType(),
                              optTy.isTrivial(subSGF.F) ? OwnershipKind::None
                                                        : OwnershipKind::Owned);
  }

  // Switch over a copy of the base.
  std::pair<EnumElementDecl *, SILBasicBlock *> caseBB(element, matchBB);
  SwitchEnumInst *switchEnum = nullptr;
  if (subjectIsAddressOnly) {
    B.createCopyAddr(loc, baseArg, subjectTmp, IsNotTake, IsInitialization);
    B.createSwitchEnumAddr(loc, subjectTmp, defaultBB, caseBB);
  } else {
    SILValue subject;
    if (baseArg->getType().isAddress()) {
      subject =
          B.emitLoadValueOperation(loc, baseArg, LoadOwnershipQualifier::Copy);
    } else {
      subject = B.emitCopyValueOperation(loc, baseArg);
    }
    switchEnum = B.createSwitchEnum(loc, subject, defaultBB, caseBB);
  }

  // A single labeled associated value is stored as a one-element labeled
  // tuple, which the surface payload type drops; project its element.
  bool payloadIsWrappedInTuple =
      hasPayload && element->getParameterList()->size() == 1 &&
      !element->getParameterList()->get(0)->getArgumentName().empty();

  // Matched case: extract the payload and wrap it in `.some`.
  B.emitBlock(matchBB);
  {
    ManagedValue payload;
    if (!hasPayload) {
      if (subjectIsAddressOnly)
        B.createDestroyAddr(loc, subjectTmp);
      payload = ManagedValue::forObjectRValueWithoutOwnership(
          B.createTuple(loc, payloadTy.getObjectType(), ArrayRef<SILValue>()));
    } else if (subjectIsAddressOnly) {
      auto payloadAddr = B.createUncheckedTakeEnumDataAddr(
          loc, subjectTmp, element, eltArgTy.getAddressType());
      if (isIndirect) {
        auto box = B.emitLoadValueOperation(loc, payloadAddr,
                                            LoadOwnershipQualifier::Take);
        SILValue inner = B.createProjectBox(loc, box, 0);
        if (payloadIsWrappedInTuple)
          inner = B.createTupleElementAddr(loc, inner, 0,
                                           payloadTy.getAddressType());
        if (payloadIsAddressOnly) {
          B.createCopyAddr(loc, inner, payloadTmp, IsNotTake, IsInitialization);
          payload = subSGF.emitManagedBufferWithCleanup(payloadTmp);
        } else {
          payload = subSGF.emitManagedRValueWithCleanup(B.emitLoadValueOperation(
              loc, inner, LoadOwnershipQualifier::Copy));
        }
        B.emitDestroyValueOperation(loc, box);
      } else {
        SILValue srcAddr = payloadAddr;
        if (payloadIsWrappedInTuple)
          srcAddr = B.createTupleElementAddr(loc, srcAddr, 0,
                                             payloadTy.getAddressType());
        if (payloadIsAddressOnly) {
          B.createCopyAddr(loc, srcAddr, payloadTmp, IsTake, IsInitialization);
          payload = subSGF.emitManagedBufferWithCleanup(payloadTmp);
        } else {
          payload = subSGF.emitManagedRValueWithCleanup(B.emitLoadValueOperation(
      ...
      // Loadable subject: the payload is the forwarded terminator result.
      auto input = B.createForwardedTermResult(eltArgTy);
      if (isIndirect) {
        SILValue box = input.forward(subSGF);
        SILValue inner = B.createProjectBox(loc, box, 0);
        if (payloadIsWrappedInTuple)
          inner = B.createTupleElementAddr(loc, inner, 0,
                                           payloadTy.getAddressType());
        ...
        B.emitDestroyValueOperation(loc, box);
      } else if (payloadIsWrappedInTuple) {
        auto *destructure =
            B.createDestructureTuple(loc, input.forward(subSGF));
        payload =
            subSGF.emitManagedRValueWithCleanup(destructure->getResult(0));
      } else {
        payload = input;
      }
    }

    ArgumentSource payloadSource[1] = {
        ArgumentSource(loc, RValue(subSGF, loc, payloadType, payload))};
    if (resultIsAddressOnly) {
      KnownAddressInitialization init(resultTmp);
      auto some = subSGF.emitInjectEnum(loc, payloadSource, optTy, someDecl,
                                        SGFContext(&init));
      if (!some.isInContext())
        some.forwardInto(subSGF, loc, resultTmp);
      B.createBranch(loc, contBB);
    } else {
      auto some = subSGF.emitInjectEnum(loc, payloadSource, optTy, someDecl,
                                        SGFContext());
      B.createBranch(loc, contBB, some.forward(subSGF));
    }
  }

  // Any other case: produce `.none`.
  B.emitBlock(defaultBB);
  {
    if (subjectIsAddressOnly) {
      B.createDestroyAddr(loc, subjectTmp);
    } else {
      // The default result is null when the default covers a unique
      // no-payload case.
      if (SILValue defaultValue = switchEnum->createDefaultResult())
        B.emitDestroyValueOperation(loc, defaultValue);
    }
    if (resultIsAddressOnly) {
      B.createInjectEnumAddr(loc, resultTmp, noneDecl);
      B.createBranch(loc, contBB);
    } else {
      SILValue none =
          B.createEnum(loc, SILValue(), noneDecl, optTy.getObjectType());
      B.createBranch(loc, contBB, none);
    }
  }
  ...
  ManagedValue resultSubst;
  if (resultIsAddressOnly) {
    resultSubst = subSGF.emitManagedBufferWithCleanup(resultTmp);
  } else {
    resultSubst = subSGF.emitManagedRValueWithCleanup(contBB->getArgument(0));
  }

  emitReturn(subSGF, componentType, resultSubst, resultArgTy, resultArg, scope,
             loc);
...
static SILFunction *getOrCreateKeyPathEnumCaseEmbed(
    SILGenModule &SGM, EnumElementDecl *element, SubstitutionMap subs,
    GenericEnvironment *genericEnv, ResilienceExpansion expansion,
    CanType baseType, CanType componentType) {
  auto genericSig =
  ...
  auto signature = createKeyPathSignature(
      SGM, genericSig, /*baseType*/ payloadType, /*propertyType*/ baseType, {},
      SILFunctionType::Representation::KeyPathAccessorGetter,
      ParameterConvention::Indirect_In_Guaranteed);

  auto name = Mangle::ASTMangler(SGM.getASTContext(), element)
                  .mangleKeyPathCaseEmbedThunkHelper(element, genericSig,
                                                     baseType, subs, expansion);
  auto loc = RegularLocation::getAutoGeneratedLocation();
  auto thunk =
      getOrCreateKeypathThunk(SGM, name, signature, genericEnv, expansion, loc);
  ...
  if (genericEnv) {
    baseType = genericEnv->mapTypeIntoEnvironment(baseType)->getCanonicalType();
    payloadType =
        genericEnv->mapTypeIntoEnvironment(payloadType)->getCanonicalType();
    thunk->setGenericEnvironment(genericEnv);
  }
  ...
  auto payloadArgTy =
      subSGF.silConv.getSILType(signature->getParameters()[0], signature,
                                subSGF.F.getTypeExpansionContext());
  SILFunctionArgument *resultArg = nullptr;
  SILFunctionArgument *payloadArg = nullptr;
  SILValue indexPtrArg;
  emitKeyPathThunk(SGM, subSGF, genericEnv, signature, thunk, {}, resultArg,
                   resultArgTy, payloadArg, payloadArgTy, indexPtrArg,
                   SILParameterInfo());

  ArgumentScope scope(subSGF, loc);

  // The payloads passed to emitInjectEnum are parallel to the case's
  // parameters.
  SmallVector<ArgumentSource, 4> payloadSources;
  if (element->hasAssociatedValues()) {
    auto payloadOrig = payloadArgTy.isTrivial(subSGF.F)
                           ? ManagedValue::forRValueWithoutOwnership(payloadArg)
                           : ManagedValue::forBorrowedRValue(payloadArg);
    payloadOrig = payloadOrig.copy(subSGF, loc);
    auto payloadSubst = subSGF.emitOrigToSubstValue(
        loc, payloadOrig, AbstractionPattern::getOpaque(), payloadType);

    RValue payloadRV(subSGF, loc, payloadType, payloadSubst);
    if (element->getParameterList()->size() == 1) {
      payloadSources.emplace_back(loc, std::move(payloadRV));
    } else {
      SmallVector<RValue, 4> eltRVs;
      std::move(payloadRV).extractElements(eltRVs);
      for (auto &elt : eltRVs)
        payloadSources.emplace_back(loc, std::move(elt));
    }
  }

  auto enumTy = subSGF.getLoweredType(baseType);
  auto resultSubst = subSGF.emitInjectEnum(loc, payloadSources, enumTy,
                                           element, SGFContext());

  emitReturn(subSGF, baseType, resultSubst, resultArgTy, resultArg, scope, loc);
...
KeyPathPatternComponent SILGenModule::emitKeyPathComponentForDecl(
  ...
  if (auto *element = dyn_cast<EnumElementDecl>(decl)) {
    auto &C = getASTContext();

    // Compute the payload type the way the type checker does: `Void` for
    // no payload, the value itself for a single associated value, and a
    // tuple otherwise.
    CanType payloadTy;
    if (auto *params = element->getParameterList()) {
      if (params->size() == 1) {
        payloadTy = params->get(0)->getInterfaceType()->getCanonicalType();
      } else {
        SmallVector<TupleTypeElt, 4> elts;
        for (auto *param : *params)
          elts.emplace_back(param->getInterfaceType(),
                            param->getArgumentName());
        payloadTy = TupleType::get(elts, C)->getCanonicalType();
      }
      // Map the payload into the substituted base type and back out to an
      // interface type, mirroring the property component path.
      payloadTy =
          payloadTy
              .subst(GenericEnvironment::mapTypeIntoEnvironment(genericEnv,
                                                                baseTy)
                         ->getContextSubstitutionMap(element->getParentEnum()))
              ->mapTypeOutOfEnvironment()
              ->getCanonicalType();
    } else {
      payloadTy = CanType(TupleType::getEmpty(C));
    }
    auto componentTy = OptionalType::get(payloadTy)->getCanonicalType();

    auto *extract = getOrCreateKeyPathEnumCaseExtract(
        *this, element, subs, needsGenericContext ? genericEnv : nullptr,
        expansion, baseTy, componentTy);

    // Deployment targets without runtime support get the back-deployed
    // form: a get-only computed component that older runtimes instantiate
    // as an ordinary read-only key path.
    bool caseKeyPathAvailable =
        C.LangOpts.DisableAvailabilityChecking ||
        AvailabilityRange::forDeploymentTarget(C).isContainedIn(
            C.getCaseKeyPathsAvailability());
    if (!caseKeyPathAvailable) {
      return KeyPathPatternComponent::forComputedGettableProperty(
          KeyPathPatternComponent::ComputedPropertyId(SILDeclRef(element)),
          extract, {}, nullptr, nullptr, nullptr, SubstitutionMap(),
          componentTy);
    }

    auto *embed = getOrCreateKeyPathEnumCaseEmbed(
        *this, element, subs, needsGenericContext ? genericEnv : nullptr,
        expansion, baseTy, componentTy);

    return KeyPathPatternComponent::forEnumCase(
        KeyPathPatternComponent::ComputedPropertyId(SILDeclRef(element)),
        extract, embed, componentTy);
  }
...
RValue RValueEmitter::visitKeyPathExpr(KeyPathExpr *E, SGFContext C) {
  ...
  for (size_t i = 0; i < components.size(); i++) {
    ...
    case KeyPathExpr::Component::Kind::Subscript: {
      ...
      // `.some` followed by optional chaining or forcing canonicalizes to
      // the chain or force alone — except in a `CaseKeyPath`, where the
      // case component carries the embed half.
      if (decl == SGF.getASTContext().getOptionalSomeDecl() &&
          !E->getType()->isCaseKeyPath() && i + 1 < components.size()) {
        auto nextKind = components[i + 1].getKind();
        if (nextKind == KeyPathExpr::Component::Kind::OptionalChain ||
            nextKind == KeyPathExpr::Component::Kind::OptionalForce)
          continue;
      }
